...
import com.tui.proof.adapters.repository.OrderRepository;
...
public class H2Adapter implements H2Port {
  ...
  public List<Order> findByClient(Client client) {
    return h2Repository.findOrdersByClientCriteria(clientMapper.toEntity(client)).stream()
  ...
  public List<Order> findAll() {
    return h2Repository.findAll().stream().map(OrderMapper.INSTANCE::toModel).toList();
  }
}
